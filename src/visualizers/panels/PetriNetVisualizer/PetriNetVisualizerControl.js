...
define([
    ...
    PetriNetVisualizerControl.prototype._initPN = function () {
        const self = this;
        ...
        elementIds.forEach(elementId => {
            ...
            if (node.isTypeOf(META['Place'])) {
                //right now we are only interested in places...
                const state = {name: node.getAttribute('name'), markings: node.getAttribute('Markings'), out: [], position: node.getRegistry('position')};
                pn.places[elementId] = state;
            }
            if (node.isTypeOf(META['Transition'])) {
                const state = {name: node.getAttribute('name'), markings: node.getAttribute('Markings'), in: [], out: [], position: node.getRegistry('position')};
                ...
                elementIds.forEach(nextId => {
                    ...
                    if(nextNode.isTypeOf(META['PlaceToTransitionArc']) && nextNode.getPointerId('dst') === elementId) {
                        state.in.push(nextNode.getPointerId('src'));
                    }
                });
                elementIds.forEach(nextId => {
                    const nextNode = self._client.getNode(nextId);
                    if(nextNode.isTypeOf(META['TransitionToPlaceArc']) && nextNode.getPointerId('src') === elementId) {
                        state.out.push(nextNode.getPointerId('dst'));
                    }
                });
                pn.transitions[elementId] = state;
            }
        });
    ...
    PetriNetVisualizerControl.prototype._initializeToolbar = function () {
        ...
        this.$btnClassificationInterpreter = toolBar.addButton({
            title: 'Check Petri net classification',
            icon: 'glyphicon glyphicon-question-sign',
            clickFn: function (/*data*/) {
                const context = self._client.getCurrentPluginContext('ClassifyPetriNet',self._currentNodeId, []);
        ...
        this._toolbarItems.push(this.$btnClassificationInterpreter);
